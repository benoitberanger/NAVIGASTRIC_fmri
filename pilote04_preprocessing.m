...
subjectDirs = get_subdir_regex(mainPath,'Pilote04');
...
examArray = exam(mainPath,'Pilote04');
...
% 
examArray.addSerie('EBA$'      , 'EBA'      ,1)
examArray.addSerie('EBA_AP$'   , 'EBA_AP'   ,1) % refAP
examArray.addSerie('MOTOR$'        , 'MOTOR'        ,1)
examArray.addSerie('MOTOR_AP$'     , 'MOTOR_AP'     ,1) % refAP

% All func volumes
examArray.getSerie({'EBA','MOTOR'}).addVolume('^f.*nii','f',1)
...
dfoncall = examArray.getSerie(regex_dfonall).toJob
...
examArray.getSerie(regex_dfonall).addVolume('^utmeanf','utmeanf',1)
examArray.getSerie(regex_dfonall).addVolume('^utrf.*nii','utrf',1)
...
for nbs=1:length(subjectDirs)
    fmean(nbs) = examArray.getSerie('EBA$').getVolume('^utmeanf').toJob
end
